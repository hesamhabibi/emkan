...
module.exports = async (parent, args, { models: { FormModel }, helpers: { ValidationHelper }, error_res, trans, AuthUser }) => {
    // get input
    const input = collect(args.input).only(['title', 'name', 'fields']).all();
    const new_fields = [];
    for (let i = 0; i < input.fields.length; i += 1) {
        new_fields.push(collect(args.input.fields[i]).only(['label', 'name', 'size', 'default_value', 'data', 'field_type_id', 'field_validation_ids']).all());
    }
    ...
    for (let i = 0; i < input.fields.length; i += 1) {
        if (!input.fields[i].name)
            input.fields[i].name = SlugHelper.convert_to_english_variable_name(extract_main_title(input.fields[i].label));
        if (Array.isArray(input.fields[i].data))
            for (let j = 0; j < input.fields[i].data.length; j += 1) {
