...
module.exports = async (parent, args, { models: { ProductModel, BrandModel, MediaModel, AttributeModel, AttributeValueModel, CategoryModel, CollectionModel }, helpers: { ValidationHelper, SEOHelper, PriceHelper, MediaHelper: { attachMedia, detachMedia, prepareMediaGallery } }, error_res, trans, AuthUser }) => {
    // find product
    let product;
    try {
    ...
    const { validation_result: seo_validation_result } = await SEOHelper.validate_input(input.seo, product);
    ...
    const result = await prepareMediaGallery(input.media_gallery, product, product.media_gallery);
    input.media_gallery = result.media_gallery;
    input.media = result.main_media;

    // prepare attribute group info
    const attribute_groups = [];
    if (input.attribute_groups) {
        // delete previous attribute values
        AttributeValueModel.deleteMany({ product_id: product._id });
    ...
    // set attributes and attribute_groups
    ...
    if (input.has_variant) {
        ...
        // detach all medias in gallery from this product
        if (product.has_variant) {
            if (Array.isArray(product.variant) && Array.isArray(product.mix_variant)) {
                for (let i = 0; i < product.mix_variant.length; i += 1) {
                    if (product.mix_variant[i].has_media_gallery) {
                        if (Array.isArray(product.mix_variant[i].has_media_gallery)) {
                            for (let i = 0; i < product.mix_variant[i].has_media_gallery?.length; i += 1) {
                                detachMedia(product, product.mix_variant[i].has_media_gallery[i].media_id);
                            }
                        }
                    }
                }
        ...
        if (Array.isArray(input.variant) && Array.isArray(input.mix_variant)) {
            for (let i = 0; i < input.mix_variant.length; i += 1) {
                ...
                const temp_mix_variant = {
                    ...
                    keys: keys,
                    price_id: null, // will be filled after this loop
                };
    ...
    // set variant and mix_variant
    let input_mix_variant = input.mix_variant;
    input.variant = variant;
    input.mix_variant = mix_variant;

    // prepare collections
    if (input.collections && input.collections.related_products) {
        // if the previous collection was static then delete it
        try {
            const previous_collection = await CollectionModel.findOne({ _id: product.collections.related_products.collection_id, type: CollectionModel.types.static });
            if (previous_collection) {
                // delete collection
                if (previous_collection?.extra_fields?.seo_id)
                    await SEOHelper.deleteSEO(previous_collection?.extra_fields?.seo_id);
                await previous_collection.delete();
            }
        } catch { /* empty */ }
    ...
    // update product
    const last_video_id = product?.video?.media_id;
    const last_files = product?.files;
    await product.set(input).save();
    ...
    if (input?.video?.media_id)
        try {
            await attachMedia(product, input.video.media_id, last_video_id);
        } catch {
            console.log('error in update media relation');
        }
    // detach all medias in files from this product
    for (let i = 0; i < last_files.length; i += 1) {
        try {
            detachMedia(product, last_files[i].media_id);
        } catch {
            console.log('error in update media relation');
        }
    }
    ...
    if (input.has_variant) {
        if (Array.isArray(product.variant) && Array.isArray(product.mix_variant)) {
            for (let i = 0; i < product.mix_variant.length; i += 1) {
                const result = await PriceHelper.createVariantPrice({ model_id: product._id, model_name: ProductModel.modelName, model_variant_keys: product.mix_variant[i].keys }, input_mix_variant[i].price);
                if (result.price) {
