...
module.exports = async (parent, args, { models: { DeputationModel }, helpers: { ValidationHelper, MediaHelper: { attachMedia, prepareMediaGallery } }, error_res, trans }) => {
    // find deputation
    let deputation;
    try {
    ...
    const result = await prepareMediaGallery(input.media_gallery, deputation, deputation.media_gallery);
    input.media_gallery = result.media_gallery;
    input.media = result.main_media;

    // update deputation
    await deputation.set(input).save();
