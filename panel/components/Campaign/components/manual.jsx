...
export default function Manual({
    ...
    const getValue = async (fields) => {
        const res = await getPage({fields, query: queries.relatedProducts,})
        setVariants(res.result)
    }

    const variantLabels = (product_id) => {
        return variants.filter((item) => item.id === product_id)
                        ...
                        setData={(data) => {
                            ...
                            data.forEach((item, index) => {
                                if (!item.key?.length)
                                    ...
                                    values.find((value) => item?.id === value?.product_id && _.includes(value?.mix_variant_keys, item?.key)).sort = index
                            })
                            setValue("list", values)
                        }}
                        section="products"
