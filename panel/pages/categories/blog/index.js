...
const Categories = () => {
  ...
  const createModal = () => {
    ...
    setTitle(translation("create blog", "category"))
  ...
  const editModal = (item) => {
    ...
    setTitle(
      translation("edit blog", "category") +
        " " +
        `"${item.title_panel ?? "-"}"`
    )
    dispatch(openModal("editForms"))
  ...
  return (
    <AdminLayout
      action={
        <Shield id="categories_blog_create_action" action>
      ...
      title={`${translation("Categories")} - ${translation("Blog")}`}
    >
