...
export default function Categories() {
  ...
  const createModal = (item) => {
    ...
    setTitle(translation("create device", "category"))
  ...
  const editModal = (item) => {
    ...
    setTitle(
      translation("edit device", "category") +
        " " +
        `"${item.title_panel ?? "-"}"`
    )
    dispatch(openModal("editForms"))
  ...
  return (
    <AdminLayout
      action={
        <Shield id="categories_device_create_action" action>
      ...
      title={`${translation("Categories")} - ${translation("Devices")}`}
    >
