...
const value_types = {
  ...
  9: ({data, control, key}) => {
    ...
    return <Input type="select-searchable" control={control} name={`wheres.${key}.where_value`} data={data}/>
  },
  10: ({data, control, key}) => {
    // console.log(data, control, 10)

    return <Input type="select-multiple" control={control} name={`wheres.${key}.where_value`} data={data}/>
  },
...
export default function Auto({control: mainControl, setCallback, setVal, ...props}) {
  ...
  const selector = useSelector(state => state.collapse.value["create-collection"])
  ...
  function renderValue(key) {
    ...
    const value = data.value.find(item => item.key === whereField)
      ...
      return (
        <>
          <label>{translation("value")}</label>
          {
            type({
              key,
              data: data.extra_data[value.dynamic_values] ||
                value.values?.map(item => ({name: item.title_panel, id: item.key})), control, getValues
            })
          }
        </>
      )
    ...
    return (
      <Input
        ...
        control={control}
        data={value.values?.map(item => ({name: item.title_panel, id: item.key}))}
        gridSize={6}
      />
    )
  }
      ...
      <Fragment key={key}>
        <Grid size={12}>
          <Input
            ...
            control={control}
            data={data.value.map(item => ({name: item.title[router.locale], id: item.key}))}
        ...
        <Grid size={12}>
          {
            (watch(`wheres.${key}.where_field`) | true) && (
              <Input
                ...
                control={control}
                data={data.value.find(item => item.key === getValues(`wheres.${key}.where_field`))
                  ?.operators.map(
                    item => ({
        ...
        <Grid size={1}>
          <Button className="px-3" type="error" onClick={removeInput.bind(this, key)}>
  ...
  function renderSorts(key) {
    return (
      <Fragment key={key}>
        ...
        <Grid size={12}>
          <Input
            ...
            data={Object.keys(orders_types).map(item =>
              ({id: item, name: translation(orders_types[item], "products")}))}
            control={control}
        ...
        <Grid size={1}>
          <Button className="px-3" type="error" onClick={removeSort.bind(this, key)}>
            <i className={`far fa-trash-alt`}/>
          </Button>
        </Grid>
      </Fragment>
  ...
  async function createCollection(data) {
    ...
    try {
      ...
      fireToast(translation("Operation Completed Successfully", { status: "success" }))
    } catch (e) {
      if (e.graphQLErrors) {
        setErrors(e.graphQLErrors[0], setError)
      }
  ...
  return (
    <>
      <GridContainer className="text-right mb-5" gap="Lg">
        <Grid size={12}>
          <Input
        ...
        <Grid className="align-self-end" size={12}>
          <Button onClick={dispatch.bind(this, toggle("create-collection"))} className="w-100" type="primary">
            {translation("Add Collection", "products")}
            <i className={`fas fa-angle-down ${Styles.icon} ${selector ? "fa-rotate-180": ""} mr-2`}/>
          </Button>
        </Grid>
      </GridContainer>


      <Collapse id="create-collection">
        <GridContainer className="mt-4 text-right align-items-end" gap="Lg">
          <Grid size={12}>
            <h4>{translation("Query and logic", "products")}</h4>
          </Grid>
          <Grid size={12}>
            <Input type="number" label={translation("limit")} control={control} name="limit"/>
          </Grid>
          <Grid size={12}>
            <Input
              type="toggle" control={control} name="logic"
              data={[{name: "AND", id: 1}, {name: "OR", id: 2}]}
              label={translation("query method")}
            />
          </Grid>
          <Grid size={12}>
            <Button type="primary" className="w-100" onClick={handleSubmit(preview)}>
              <i className="fas fa-eye ml-3"/>
              {translation("preview")}
            </Button>
          </Grid>
          <Grid size={12}>
            <Button type="success" onClick={handleSubmit(createCollection)} className="w-100">
              {translation("Submit")}
              <i className="mr-2 fas fa-cloud-upload"/>
            </Button>
          </Grid>
          <Grid className={Styles.form} size={12}>

            <h4 className="mb-5">{translation("Query Combos", "products")}</h4>

            <GridContainer className="align-items-end" gap="Lg">
              {watch("wheres") && getValues("wheres").map((item, key) => renderInputs(key))}
              {/*<Grid size={9}/>*/}
              <Grid size={12} className="text-left">
                <Button onClick={addCondition} type="success" className="mt-4 w-100">
            ...
            <GridContainer className="align-items-end" gap="Lg">
              {watch("orders") && getValues("orders").map((item, key) => renderSorts(key))}
              <Grid size={12} className="text-left">
                <Button onClick={addSort} type="success" className="w-100 mt-4">
                  <i className="fas fa-plus-circle ml-1"/>
          ...
          <Grid className={`${Styles.result} align-self-start`} size={12}>
            <Table
              data={previewData}
              fields={[
                {
                  title: "image", td: (row) =>
                    <img width="50" src={row.media?.url || "/images/not-found.png"} alt={row.media?.alt}/>
                },
                {title: "product_code", td: (row) => row.mix_variant[0].details.product_code},
