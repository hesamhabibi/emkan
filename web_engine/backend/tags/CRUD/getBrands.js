...
module.exports = {
    ...
    render: async function (ctx) {
        ...
        for (let i in keys) {
            ...
            let query = '';
            query += `
                query ($filter: BrandFilter,$sort: BrandSort,$page:Int,$limit: Int) {
                    result: getBrands(filter:$filter,sort:$sort,page:$page,limit:$limit){
                        ${fields}
                    }
                }
            `;
            ...
            if (result === undefined)
                console.log(`no result for query "getBrands" for ${keys[i]}`);
