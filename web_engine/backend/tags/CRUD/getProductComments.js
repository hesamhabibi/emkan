...
module.exports = {
    ...
    render: async function (ctx) {
        ...
        for (let i in keys) {
            ...
            let query = `
                query ($model_id: ID, $page: Int, $limit: Int, $filter: CommentFilter, $sort: CommentSort) {
                    result: getProductComments(model_id: $model_id, page: $page, limit: $limit, filter: $filter, sort: $sort){
                        ${fields}
                    }
                }
            `;
            ...
            if (result === undefined)
                console.log(`no result for query "getProductComments" for ${keys[i]}`);
