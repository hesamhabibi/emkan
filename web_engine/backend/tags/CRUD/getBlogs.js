...
module.exports = {
    ...
    render: async function (ctx) {
        ...
        for (let i in keys) {
            ...
            let query = '';
            query += `
                query ($filter: BlogFilter,$sort: BlogSort,$page:Int,$limit: Int) {
                    result: getBlogs(filter:$filter,sort:$sort,page:$page,limit:$limit){
                        ${fields}
                    }
                }
            `;
            ...
            if (result === undefined)
                console.log(`no result for query "getBlogs" for ${keys[i]}`);
