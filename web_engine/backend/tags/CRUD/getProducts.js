...
module.exports = {
    ...
    render: async function (ctx) {
        ...
        for (let i in keys) {
            ...
            let query = '';
            query += `
                query ($filter: ProductFilter,$page:Int,$limit: Int,$sort: ProductSort) {
                    result: getProducts(filter:$filter,page:$page,limit:$limit,sort:$sort){
                        ${fields}
                    }
                }
            `;
            ...
            if (result === undefined)
                console.log(`no result for query "getProducts" for ${keys[i]}`);
            try {
