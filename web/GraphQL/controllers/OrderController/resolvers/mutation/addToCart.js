...
module.exports = async (parent, args, { models: { OrderModel }, helpers: { ValidationHelper }, error_res, trans, AuthUser }) => {
    ...
    if (!userOrder) {
        ...
        if (order) {
            let product = order.products.find(function (el) {
                return String(el.product_id) == String(input.product_id) && String(input.mix_variant_keys) == String(el.mix_variant_keys);
            });

            try {
                let countResult = product.count + input.count;
                if (countResult < 0)
                    countResult = 0;
                ...
                const updatedOrder = await OrderModel.findOneAndUpdate(
                    ...
                    {
                        $set: { 'products.$.count': countResult }
                    },
                    {
                        new: true
                    }
